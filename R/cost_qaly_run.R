cost_qaly_run=function(X,scale00,scale10,discount_rate){
...
# qaly=calc_qalyv2(YSPONT,utilsof$`Spontaneous viral clearance, never infected`,3)
# YA=(1-delta)*(colSums(scale00*X[C$A_comps_00,])+colSums(scale10*X[C$A_comps_10,]))
# qaly=qaly+calc_qalyv2(YA,utilsof$`Acute HCV`,3)

r_qaly=rep(0,11) # These are the stage values
r_stage_cost=rep(0,11)
YS0=(colSums(scale00*X[C$S0_comps_00,])+colSums(scale10*X[C$S0_comps_10,]))
qaly=calc_qalyv2(YS0,utilsof$`Spontaneous viral clearance, never infected`,discount_rate)
r_qaly[1]=sum(qaly)
YA=(colSums(scale00*X[C$A_comps_00,])+colSums(scale10*X[C$A_comps_10,]))
dum=calc_qalyv2(YA,utilsof$`Acute HCV`,discount_rate)
r_qaly[2]=sum(dum)
qaly=qaly+dum
YF0=(colSums(scale00*X[C$F0_comps_00,])+colSums(scale10*X[C$F0_comps_10,]))
dum=calc_qalyv2(YF0,utilsof$`Mild chronic HCV (F0/F1/F2)`,discount_rate)
r_qaly[3]=sum(dum)
qaly=qaly+dum
YF1=(colSums(scale00*X[C$F1_comps_00,])+colSums(scale10*X[C$F1_comps_10,]))
dum=calc_qalyv2(YF1,utilsof$`Mild chronic HCV (F0/F1/F2)`,discount_rate)
r_qaly[4]=sum(dum)
qaly=qaly+dum
YF2=(colSums(scale00*X[C$F2_comps_00,])+colSums(scale10*X[C$F2_comps_10,]))
dum=calc_qalyv2(YF2,utilsof$`Mild chronic HCV (F0/F1/F2)`,discount_rate)
r_qaly[5]=sum(dum)
qaly=qaly+dum
YF3=(colSums(scale00*X[C$F3_comps_00,])+colSums(scale10*X[C$F3_comps_10,]))
dum=calc_qalyv2(YF3,utilsof$`Moderate chronic HCV (F3)`,discount_rate)
r_qaly[6]=sum(dum)
qaly=qaly+dum
YF4=(colSums(scale00*X[C$F4_comps_00,])+colSums(scale10*X[C$F4_comps_10,]))
dum=calc_qalyv2(YF4,utilsof$`Compensated cirrhosis (F4)`,discount_rate)
r_qaly[7]=sum(dum)
qaly=qaly+dum
YDC=(colSums(scale00*X[C$DC_comps_00,])+colSums(scale10*X[C$DC_comps_10,]))
dum=calc_qalyv2(YDC,utilsof$`Decompensated cirrhosis/liver failure`,discount_rate)
r_qaly[8]=sum(dum)
qaly=qaly+dum
YHCC=(colSums(scale00*X[C$HCC_comps_00,])+colSums(scale10*X[C$HCC_comps_10,]))
dum=calc_qalyv2(YHCC,utilsof$`Hepatocellular carcinoma`,discount_rate)
r_qaly[9]=sum(dum)
qaly=qaly+dum
YLT1=(colSums(scale00*X[C$LT1_comps_00,])+colSums(scale10*X[C$LT1_comps_10,]))
dum1=calc_qalyv2(YLT1,utilsof$`Liver transplantation year 1`,discount_rate)
qaly=qaly+dum1
YLT2=(colSums(scale00*X[C$LT2_comps_00,])+colSums(scale10*X[C$LT2_comps_10,]))
...
#print(paste0("here here",discount_rate))

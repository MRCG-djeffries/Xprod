run_gen_pop_v7=function(treat_cov_mild,treat_cov_severe,
                        screening_cov,screening_diagnosis_duration,
                        ignore_screening,time_ofint_inODEunits,runy,
                        version_num,
                        treatduration){
  # fits to increased risk from 1970 to 1990
  # corrected error in Scott paper for parameters for svr to DC and HCC (affect will be seen for treat)
  # fit with run_gen_pop_v7(0,0,0,1,0,7,1,8,52/12) # fit
  # run with run_gen_pop_v7(0,0,0,1,0,7,0,8,52/12) # fit
  library(deSolve)
  library(scales)
  library(cowplot)
  library(ggplot2)
  ...
  # pick DAA data  was hard coded pcom=0.892,alpha=0.95
  dum=pickupDAA_efficacy_gen()
  pcom=dum$pcom
  alpha=dum$alpha_genpop
  
  
  if (runy==1){
    Nint<<-fitN_v4(1,0.2,1400)
    solveGA_US_gen_pop_v3(version_num)
    #dbDisconnect(conn)
    x=readRDS(paste0("www/genpop_fit_raw_v",version_num,".rds"))
    param_vals=param_setup_gen_pop(x,1)
    dum=runmodel_fit_genpop(param_vals,x,0,0,0,1,0,7,0,0,0)
  ...
  plotmort=plot_output_mortality_v2_all_genpop(dum$X,param_vals[913:916],7)
  plotcasepertreat=plot_output_cases_averted_all_genpop(0:35,L,Xnotreat,dum$X,pcom,alpha,7,treat_cov_mild,treat_cov_severe,
                                                        screening_cov,screening_diagnosis_duration,
                                                        ignore_screening)
  ...
  plotmort_num = plot_output_mortality_v2_all_num_genpop(dum$X,param_vals[913:916],7)
  plotstages=p1
  plotattackrate=p3
  plotattackrate_per_treat=plot_attack_rate_v_treat_all_genpop(0:35,L,Xnotreat,dum$X,pcom,alpha,7,treat_cov_mild,treat_cov_severe,
                                                               screening_cov,screening_diagnosis_duration,
                                                               ignore_screening)
  plotprevent_count=plot_output_screened_count_genpop(dum$X,7)
  # p11=plot_allcomps_v3(0:90,X,scale10,L)
  # p12=plot_output_pwid_all(0:90,L,X,scale00,scale10,TreatmentStartTime,1)
  plotinci_num_data = plotincidence_v2_noscaling_data(dum$X,L,7,delta,acuterate,curr_mort_pwid,curr_mort_former)
  plotmort_num_data = plot_output_mortality_v2_all_num_genpop_data(dum$X,1-exp(-param_vals[913:916]),7)
  ...
  return(list(p1=p1,p2=p2,p3=p3,p4=p4,p5=p5,
              ...
              plotinci_num_data=plotinci_num_data,
              plotmort_num_data=plotmort_num_data,
              plot_both=plot_both
  ))
...
solveGA_US_gen_pop_v3 = function(version_num){
  # fit allowing for the baby boomer effect
  
  library(GA)
  library(deSolve)
  x=readRDS(paste0("www/genpop_fit_raw_v",(version_num-1),".rds"))
  svals =c(0.0,rep(0.01,10))
  evals=c(0.0,0.3,rep(0.2,9))
  GA <- ga(type = "real-valued", fitness = asode_US_genpop_v3, lower = svals, upper = evals,monitor=TRUE,
           parallel=FALSE,suggestions=x,maxiter=10)
...
asode_US_genpop_v3=function(x){
  ...
  times      = seq(0, 34, by = 1)
  lt=length(times)
  X = ode(y=init, times=times, func=MODEL_US_gen_pop_v3, parms=parameters,method="ode45")
  X=t(X[,2:181])
  ...
  acute = colSums(X[A_comps_00,])
  tot = f0+f1+f2+f3+f4+hcc+dc+lt
  #tx=28:35
  tx2=7:35
  acute=acute[tx2]
  #fout2=f0[tx]+f1[tx]+f2[tx]+f3[tx]+f4[tx]+hcc[tx]+dc[tx]+lt[tx]
  #b=lm(fout2~tx)[[1]][2]
  c=lm(acute~tx2)[[1]][2]
  f=-(c^2)
  return(f)
}
asode_US_genpop_v2=function(x){
  # In general population 20 compartments for each age group
  # only S0, A, F0-F4,DCC,HCC and LT1 and LT2 have subjects
...
MODEL_US_gen_pop_v3 <- function(time, state, parameters) {
  ...
  I = 0;
  #I = 0.67 # not an infectious disease
  ...
  k = seq(1,161,20) # rows for S0 of ll ages
  d00=Mdash%*%X00-mort_former*X00+t(age_matrix%*%t(X00))
  
  d00[1,]=d00[1,]-c(200,200,200,200,200,100,100,100,100) # remove fixed number each year from susceptible
  d00[6,]=d00[6,]+c(200,200,200,200,200,100,100,100,100) # add into acute
...
fitN_v2=function(mult,proby){
  ...
  LT2_comps_00 = seq(15,180,20)
  chronic00=c(F0_comps_00,F1_comps_00,F2_comps_00,F3_comps_00,F4_comps_00,DCC_comps_00,HCC_comps_00,LT1_comps_00,LT2_comps_00)
  # the order is age grp 1 to age grp 9 for F0
  #.....................................for F1 
  #etc
  ...
  # redistribute so that 55-65 and 65 to 75 F2,F3,F4 have 5 times others
  oldies=c(109,129,110,130,111,131)
  toty = sum(N[chronic00])
  sum_soldies = sum(N[oldies])
  sum_others = sum(N[setdiff(chronic00,oldies)])
  N[oldies] = N[oldies]*5*sum_others/sum_soldies
  # re-normalize to same total
  N[chronic00]=toty*N[chronic00]/sum(N[chronic00])
...
fitN_v3=function(mult,proby){
  ...
  X=readRDS("www/Jgenpop_notreat_v4.rds")
  # put 2000 in acute
  acute_tot=sum(X[seq(6,166,20)])
  X[seq(6,166,20)]=2000*X[seq(6,166,20)]/acute_tot
  #X=readRDS("www/starter_for_gen_pop.rds") 
  X=matrix(X,nrow = length(X),ncol=1)
  pop_in_gen_pop_with_hcv = proby*3500000
  # In general population 20 compartments for each age group
...
fitN_v4=function(mult,proby,num_inc){
  # starting value for gen pop
  N=rep(0,180)
  # fit the general pop
  ...
  fit_year =1#67 # 2016, 1950+(0:80)[67]
  # This is from runmodel_v3(1), just taking first 180 (all former) for col 67
  X=readRDS("www/genpop_start_scenario122.rds")#X=readRDS("www/Jgenpop_notreat_v4.rds")
  ...
  # redistribute so that 55-65 and 65 to 75 F2,F3,F4 have 5 times others
  oldies=c(109,129,110,130,111,131) # F2,F3 and F4 compartments for older subjects
  toty = sum(N[chronic00])
  sum_soldies = sum(N[oldies])
  sum_others = sum(N[setdiff(chronic00,oldies)])
  N[oldies] = N[oldies]*5*sum_others/sum_soldies
  ...
  # susceptible
  # ratioof=sum(X[S_comps_00,1])/sum(X)
  # N[S_comps_00]=ratioof*sum(N)
  # N = pop_in_gen_pop_with_hcv*N/sum(N)
  
  age_weights = X[S_comps_00,1]/sum(X[S_comps_00,1])
  ratioof=sum(X[S_comps_00,1])/sum(X)
  N[S_comps_00]=ratioof*sum(N)*age_weights

  # add in the acute
  # put num_inc in acute
  # approx 30 weeks in acute
  # so expect 30.3/52 * num_inc coming into acute
  # approx 30% sponatenous clear
  acute_tot = num_inc * 30.3/52 * 0.7
  renorm = sum(N[seq(6,166,20)])-acute_tot
  N[setdiff(1:180,seq(6,166,20))]=N[setdiff(1:180,seq(6,166,20))]*(1+renorm/sum(N[setdiff(1:180,seq(6,166,20))]))
  # These are distributed as c(2,2,2,2,2,1,1,1,1)
  normalised_acute_tot = c(2,2,2,2,2,1,1,1,1) * acute_tot/14
  N[seq(6,166,20)]=normalised_acute_tot
  N=1.045*N # to allow for 1 year behind 2016
  return(N)
}
...
plotstacked_v3_noscaling=function(X,L){
  ...
  df$Stage=factor(df$Stage,levels=c("F0","F1","F2","F3","F4","DC","HCC","LT"))
  p1=ggplot(data=subset(df,times>=7 & times<=25), aes(x=t+2015, y=y,fill=Stage)) + geom_area() +
    xlab("Year") + ylab("Population") + ggtitle(paste0("Number in chronic compartments"))+
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),expand=c(0,0),limits=c(0,maxy),breaks=vecy)+
    scale_x_continuous(breaks = c(seq(2022,2040,1)),limits=c(2022,2040),expand = c(0,0))+
    theme(plot.background = element_rect(fill = "white"),axis.line = element_line(colour = "black"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
}
...
plotPWID_noscaled=function(XT,L,time_cut,leny){
  ...
  if (leny==1){
    tend = time_cut+19
    tdum=2014+(time_cut:(tend))
    bt=tdum
  }else{
    tend=dim(XT)[2]-1
    tdum=2014+(time_cut:(tend))
    bt=c(seq(2022,2040,1))
  } # starts at zero
...
plot_prev_v4_noscaling=function(X,L,time_cut){
  times      = seq(0, 35, by = 1)
  tend=dim(X)[2]-1
  time_cut=time_cut+2 # 8th column is 2022
  y=100*colSums(X[c(L$chronic00,L$chronic01),])/colSums(X[1:180,])
  df=data.frame(times,y)
  maxy=max(pretty_breaks()(c(0, df$y)))
  vecy=pretty_breaks()(c(0, df$y))
  p1=ggplot(data=subset(df,times>=8 & times<=26), aes(x=times+2014, y=y)) + geom_line(color="red",size=2) +
...
plotincidence_v2_change_noscaling=function(XT,L,time_cut,delta,acuterate,curr_mort_pwid,curr_mort_former){
  ...
  # integrate
  t=time_cut+2 # col 8 is 2022
  tend = 26
  yinci_PWID=T00[t:tend]+T01[t:tend]
  yinci_PWID_bot=T00[4]+T01[4] # allow time for incidence to run in
  df= data.frame(x=2014+(t:tend),y=100*((yinci_PWID/yinci_PWID_bot - 1)))
  maxy=max(pretty_breaks()(c(0, df$y)))
  vecy=pretty_breaks()(c(0, df$y))
  
  p1=ggplot(data=df, aes(x=x, y=y)) + geom_line(color="red",size=2) +
    xlab("Year") + ylab("Change %") + ggtitle("% incidence change compared to 2015")+
...
plotincidence_v2_noscaling=function(XT,L,time_cut,delta,acuterate,curr_mort_pwid,curr_mort_former){
  ...
  df= data.frame(x=2014+(t:tend),y=yinci_PWID)
  maxy=max(pretty_breaks()(c(0, df$y)))
  vecy=pretty_breaks()(c(0, df$y))
  ...
  p1=ggplot(data=df, aes(x=x, y=y)) + geom_line(color="red",size=2) +
    ...
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),expand=c(0,0),limits=c(0,maxy),breaks=vecy)+
    scale_x_continuous(breaks = c(seq((2014+time_cut),2040,1)),expand=c(0,0))+
    theme(plot.background = element_rect(fill = "white"),axis.line = element_line(colour = "black"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),plot.title = element_text(size=10))
  return(p1)
}
...
plotincidence_v2_noscaling_data=function(XT,L,time_cut,delta,acuterate,curr_mort_pwid,curr_mort_former){
  

  T00=as.vector((1-curr_mort_former)%*%XT[L$A_comps_00,])
  T01=as.vector((1-curr_mort_former)%*%XT[L$A_comps_01,])

  yinci=(1-delta)*(T00+T01)*acuterate
  return(yinci)
}
...
plot_output_incidence_new_chronic_cases_v2_all_genpop=function(XT,delta,rateA_F0,time_cut,
  ...
  tend = dim(XT)[2]-10
  yinci_PWID=T00[t:tend]+T01[t:tend]
  yinci_PWID_bot=T00[4]+T01[4] # allow for burn in
  df= data.frame(x=2014+(t:tend),y=100*((yinci_PWID/yinci_PWID_bot) - 1))
  maxy=max(pretty_breaks()(c(0, df$y)))
  miny=min(-10,min(pretty_breaks()(c(0, df$y))))
  vecy=pretty_breaks()(c(miny, df$y))
  p1=ggplot(data=df, aes(x=x, y=y)) + geom_line(color="red",size=2) +
    xlab("Year") + ylab("Change %") + ggtitle("% change in new chronic infections compared to 2015")+
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),expand=c(0,0),limits=c(-10,10))+
    scale_x_continuous(breaks = c(seq(2014+t,2015+tend,1)),expand=c(0,0))+
    theme(plot.background = element_rect(fill = "white"),axis.line = element_line(colour = "black"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  return(p1)
}
...
plot_output_treats_v2_all_genpop=function(t1,XT,pcom,alpha,time_ofint_inODEunits,
  ...
  tend = dim(XT)[2]-10
  tdum=2015+t1[time_ofint_inODEunits:(tend-1)]
  ydum=totaltestsF4[time_ofint_inODEunits:(tend-1)]+totaltests_notF4[time_ofint_inODEunits:(tend-1)]
  avy = round(1000*sum(ydum)/sum(totP[time_ofint_inODEunits:(tend-1)]))
  df= data.frame(x=tdum,y=ydum)
  p1=ggplot(data=df, aes(x=x, y=y)) + geom_line(color="red",size=2) +
    xlab("Year") + ylab("# of DAA") + ggtitle(paste0("Total number of DAA treatments - average ",avy," per 1,000 subjects"))+
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),expand=c(0,0))+
...
plot_output_mortality_v2_all_genpop =function(XT,death_vec,time_cut){
  ...
  for (i in 1 : 4){
    ...
    XTdeaths=XTdeaths+Ytest+Y1test
  }
  #print(XTdeaths)
  XTdeaths_bot=XTdeaths[4]
  XTdeaths=XTdeaths[time_cut:time_end]

  df= data.frame(x=2014+(time_cut:time_end),y=100*((XTdeaths/XTdeaths_bot) - 1))
  maxy=max(pretty_breaks()(c(0, df$y)))
  miny=min(0,min(pretty_breaks()(c(0, df$y))))
  vecy=pretty_breaks()(c(0, df$y))
  p1=ggplot(data=df, aes(x=x, y=y)) + geom_line(color="red",size=2) +
    xlab("Year") + ylab("Change %") + ggtitle("% change in liver related mortality (DCC,HCC,LT) compared to 2015")+
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),expand=c(0,0),limits=c(miny,maxy),breaks=vecy)+
    scale_x_continuous(breaks = c(seq(2014+time_cut,2014+time_end,1)),expand=c(0,0))+
    theme(plot.background = element_rect(fill = "white"),axis.line = element_line(colour = "black"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  return(p1)
}
...
plot_output_mortality_v2_all_num_genpop =function(XT,death_vec,time_cut){
  ...
  #print(XTdeaths)
  df= data.frame(x=2014+t,y=XTdeaths)
  maxy=max(pretty_breaks()(c(0, df$y)))
  miny=min(0,min(pretty_breaks()(c(0, df$y))))
  vecy=pretty_breaks()(c(0, df$y))
  p1=ggplot(data=df, aes(x=x, y=y)) + geom_line(color="red",size=2) +
    ...
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),expand=c(0,0),limits=c(miny,maxy),breaks=vecy)+
    scale_x_continuous(breaks = 2014+t,expand=c(0,0))+
    theme(plot.background = element_rect(fill = "white"),axis.line = element_line(colour = "black"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  return(p1)
}

plot_output_mortality_v2_all_num_genpop_data =function(XT,death_vec,time_cut){
  # death_vec=c(death_rate_dc,death_rate_hcc,death_rate_lt1,death_rate_lt2)
...
plot_attack_rate_v_treat_all_genpop=function(t1,L,X,XT,pcom,alpha,time_cut,
  ...
  for (i in 1 : length(t1)){
    N=XT[,i]
    t=t1[i]
    if (t>=(time_cut)) { # year 2022 and onwards
      enroll_S=screening_cov
      if  (ignore_screening==1){
        enroll_S=0
      }
...
plot_output_screened_count_genpop=function(XT,time_ofint_inODEunits){
  # XT is the output from the compartment model
  # t1 is the required times
  # This is for all intervention model
...
pickupDAA_efficacy_gen=function(){
  ...
  UNION
  select value,'p2' as 'val'  from parameter_data where Parameter=='Proportion treated who reach SVR' and Stratum=='Gen Pop'
   order by val"
  params=dbGetQuery(conn_param, strsql)
  dbDisconnect(conn_param)
  return(list(pcom=params$Value[1],alpha_genpop=params$Value[2]))
}
